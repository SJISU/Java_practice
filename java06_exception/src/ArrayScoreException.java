...
public class ArrayScoreException {
	...
	public static void main(String[] args) {
		...
		try {
			...
			int cnt = Integer.parseInt(scan.nextLine()); //-----------예외
			...
			for(int i=0;i<cnt; i++){ //0,1,2 <-행번호
				...
				for(int j=0; j<title.length; j++){ //0,1,2 <-열번호
						try {
							...
							jumsu[i][j]= Integer.parseInt(scan.nextLine());//-------------------예외
							
							//점수의 범위가 0~100사이가 아니면 예외발생...MyException 예외를 발생시킴..
							if(jumsu[i][j]<0 || jumsu[i][j]>100) { //점수가 0~100사이가 아닐때
	...
	//						else if(j==2) msg = "수학";
	//						//강제 예외 발생 
	//						throw new MyException(msg+"점수는 0~100사이의 값이어야 합니다.");
							...
							//다시 해당과목의 점수를 입력받기위해서 j의 감소
							j--; //그과목의 다시받아야하는데 그냥올라가면 그다음과목의 점수를 입력받게되니간 -해준다..
						}catch(NumberFormatException nfee) {  //NumberFormat 예외가 발생하면 아래로 바로 내려가기때문에 
															  //점수다시받아주기 위해서 여기서 다시 예외처리해준다...
			...
			System.out.println("학생수는 또는 점수는 0보다 큰 정수이어야 합니다");
		}
