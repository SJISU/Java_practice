...
public class JListTest extends JFrame implements ActionListener{
	...
	String data[]= {"사과","포도","망고","파인애플","바나나","체리","키위","오렌지","딸기","수박"};
	String data2 = "장미/백합/튤립/코스모스/맨드라미";
	public JListTest(){
		
		//목록만들기
		for(int i=0; i<data.length; i++) {
			leftModel.addElement(data[i]); //목록만들어짐
		}
		//목록만들기
	...
	//						 원래 데이터		 옮길곳
	public void listDataChanged(JList orgList, JList tarList) { //get해서 add하고 원래꺼 지운다-> data는 모델에 담겨있음-모델을 리스트에 넣어줌
		//목록을 구하는 메소드 JList- getModel,getSelected
		//목록을 추가하는 메소드  Model 메소드사용-
		//원래있던거 지우는 메소드 Model 메소드사용-removeElemnet(object obj) 
