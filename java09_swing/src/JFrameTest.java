...
public class JFrameTest extends JFrame {
	...
	public JFrameTest(){
		...
		//라벨에 배경색 정하기
		//투명처리 해제메소드 .setOpaque() ->해제를 해야지 배경색이 나온다
		...
		//Font클래스 : 글꼴(굴림체,돋음체,Arial...), 글자유형(기울임, 진하게,보통), 글자크기설정(px)
		//이 클래스는 객체생성할수없음 -> 생성메소드가 protected형이라서 -> 굳이 쓰고싶을때는 상속받아서 ->protected형아닌다른생성메소드쓸거
		//Font(String name , int style, int size) 글꼴 글자유형 글자크기 => static변수 => 클래스.변수로 사용가능
		Font fnt = new Font("궁서체", Font.ITALIC+Font.BOLD, 40);
		lb1.setFont(fnt); //라벨의 글자에 적용시킴		
		...
		//밀리초 만큼 일시정지 //잠시 쉼 -> 코드를 잠시 쉬게		
		try {
			Thread.sleep(5000); //5초 //밀리초
		}catch(InterruptedException e) {}
		
		//버튼의 라벨,Label의 라벨을 변경하기 
		//.setText() ->abstractButton의 메소든
		btn1.setText("변경된 버튼라벨");
		lb1.setText("변경된라벨");		
